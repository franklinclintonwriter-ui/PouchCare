...
| **CI/CD**       | `.gitlab-ci.yml` · [Pipelines](https://gitlab.com/Pouchcare/OS/-/pipelines) |

**SSH:** create a key (`ssh-keygen -t ed25519`), add the public key under GitLab → _Preferences_ → _SSH Keys_, then run `ssh -T git@gitlab.com`.
...
First install on a server: run `deploy.sh` (see script header for GitLab deploy key / token). Updates: `deploy/update.sh`. Nginx samples: `deploy/nginx/`.
...
- `apps/landing/docs/UI_MASTER_INDEX.md` — start here
- `apps/landing/docs/TASKS_MOCK_CRUD_MATRIX.md` — mock data CRUD coverage
- `apps/landing/docs/TASKS_PROFILE_SECURITY.md` — profile & security pages
- `apps/landing/docs/TASKS_WEB_TO_APK.md` — Web→APK service
- `apps/landing/docs/TASKS_RESPONSIVE_AUDIT.md` — responsive checklist
