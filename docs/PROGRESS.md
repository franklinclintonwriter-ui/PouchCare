...
- **Active branch:** `ent/p1-mysql` (PR-1.4 #11, stacked on `ent/p1-drop-supabase-2` #10)
- **Last merged enterprise PR:** #4–#9 in `enterprise/main` @ 27fe7d6. PR-1.3b (#10) in review.
- **Next action:** PR-1.5 (seed MySQL-compat audit + `DEPLOY-MYSQL.md` runbook), then PR-1.6 (docker-compose + CI → MySQL). **Owner:** run `cd apps/api && npx prisma migrate dev --name 0_init` on a dev MySQL to generate the committed initial migration (see `apps/api/prisma/MIGRATION_NOTES.md`) + provide `DATABASE_URL`(mysql) + R2 `S3_*`.
- **Known-broken / notes:** `apps/api` + `apps/management` tsc 0 errors; schema is valid for MySQL (`prisma validate` ✓, `generate` ✓). `0_init` is generated by the owner (needs a live MySQL — not runnable in sandbox). PR #3 (service picker) is a separate, unrelated PR.
