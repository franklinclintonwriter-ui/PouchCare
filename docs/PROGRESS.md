...
- **Active branch:** `ent/p1-seed-mysql` (PR-1.5 #12, stacked on `ent/p1-mysql` #11)
- **Last merged enterprise PR:** #4–#9 in `enterprise/main` @ 27fe7d6. PR-1.3b (#10), PR-1.4 (#11), PR-1.5 (#12) in review.
- **Next action:** PR-1.6 (docker-compose `postgres:16`→`mysql:8` + `.env.example` + CI), stacked on `ent/p1-seed-mysql`. **Owner:** generate `0_init` (`cd apps/api && npx prisma migrate dev --name 0_init`) on a live MySQL — see `MIGRATION_NOTES.md` / `docs/DEPLOY-MYSQL.md`.
- **Known-broken / notes:** `apps/api` + `apps/management` tsc 0 errors; schema valid for MySQL. `seed.ts` audited MySQL-compatible (no change). `0_init` generated by the owner (needs live MySQL). PR #3 (service picker) is separate.
