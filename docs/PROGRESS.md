...
- **Active branch:** none. **Copilot PRs open off `enterprise/main`:** **#17** (PR-2.2 audit coverage) + **#20** (PR-2.6 vitest/Playwright + CI). Both to review/merge when ready.
...
- **Next action (AI):** review/merge Copilot **#17** + **#20** when ready (resolve the shared-ledger conflict at each flatten, as with #18/#19). Then **Phase 3** feature waves — start with **PR-3.1** (Leave quotas/balances/accrual — model is AI/critical-path) and **PR-3.2** (multi-stage approval engine, AI). Once all Phase-2 schema is merged, **owner generates the single `0_init`** + sets `DATABASE_URL`(mysql)/R2 secrets. **Note:** Cursor Bugbot Autofix being disabled by owner — Bugbot reviews stay, AI owns fixes.
