...
/** Login/register/refresh can return 401 — that must not trigger the refresh-token flow. */
function isPortalPublicAuthUrl(config: InternalAxiosRequestConfig | undefined) {
  const path = `${config?.baseURL ?? ""}${config?.url ?? ""}`;
  return (
    path.includes("/portal/login") ||
    path.includes("/portal/register") ||
    path.includes("/portal/refresh") ||
    path.includes("/portal/forgot-password") ||
    path.includes("/portal/reset-password") ||
    path.includes("/portal/verify-email")
  );
}
...
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (
      ...
      !isPortalPublicAuthUrl(originalRequest)
    ) {
