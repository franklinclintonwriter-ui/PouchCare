...
export function SectionLabel({ children, className }: SectionLabelProps) {
  return (
    <p
      className={cn(
        'text-sky-400 text-xs font-bold tracking-[0.15em] uppercase mb-3',
        className,
      )}
    >
      {children}
...
export function SectionHeading({ children, className, as: Tag = 'h2' }: SectionHeadingProps) {
  return (
    <Tag
      className={cn(
        'font-sora font-bold text-slate-50 leading-tight tracking-tight',
...
export function SectionSub({ children, className }: SectionSubProps) {
  return (
    <p className={cn('text-slate-400 leading-relaxed text-base sm:text-lg', className)}>
      {children}
    </p>
  );
}
