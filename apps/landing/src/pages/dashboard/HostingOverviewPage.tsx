 ...
 * Portal API `/v1/portal/hosting` — not staff `/v1/assets/domains`.
...
export default function HostingOverviewPage() {
  ...
  const sslOk = domains.filter((d) => {
    if (d.status !== "active") return false;
    const ssl = parseDate(d.sslExpiresAt);
    return ssl ? ssl > now : false;
  }).length;
  // Previously `[...domains].sort(...).` on a Date parse; if any `expiresAt`
  // was invalid (empty string, bad format) NaN propagated through the sort
  // and corrupted the ordering silently. Now we parse + filter first so only
  // valid dates reach the sort and the panel falls back to `null` if none.
  const nearestExpiry = domains
    .filter((d) => d.status === "active")
    .map((d) => ({ domain: d, exp: parseDate(d.expiresAt) }))
    .filter((x): x is { domain: typeof x.domain; exp: Date } => x.exp !== null)
    .sort((a, b) => a.exp.getTime() - b.exp.getTime())[0]?.domain;
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        ...
        <StatCard
          label="Next renewal"
          value={nearestExpiry ? formatDateShort(nearestExpiry.expiresAt) : "—"}
      ...
      <ConfirmDialog
        ...
        onCancel={() => setMigrationConfirmOpen(false)}
        onConfirm={() => {
          setMigrationConfirmOpen(false);
          window.location.assign("mailto:support@pouchcare.com");
        }}
        title="Open your email app?"
        description="This will open a new message to support@pouchcare.com in your default email client."
