...
function JobTableRow({ job, onDelete }: { job: ApkJob; onDelete?: () => void }) {
  // Poll only while the job is still building; stop as soon as it's terminal.
  // `undefined` disables the inner useQuery so the polling loop releases when
  // the user navigates away or the job settles.
  const polling = job.status === "queued" || job.status === "processing";
  const jobDetail = useApkJob(polling ? job.id : undefined);
  const currentJob: ApkJob = (jobDetail.data as ApkJob) ?? job;
...
function JobMobileCard({ job, onDelete }: { job: ApkJob; onDelete?: () => void }) {
  const polling = job.status === "queued" || job.status === "processing";
  const jobDetail = useApkJob(polling ? job.id : undefined);
  const currentJob: ApkJob = (jobDetail.data as ApkJob) ?? job;
...
export default function WebToApkPage() {
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <HelpText className="mt-1.5">
                Applies to this new conversion only. Existing jobs keep their original plan.
              </HelpText>
            </div>
          </div>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {jobs.data.items.map((o) => (
                    <JobTableRow
                      key={o.id}
                      job={o}
                      onDelete={() => setDeleteConfirmJobId(o.id)}
                    />
                  ))}
                </tbody>
              </table>
      ...
      <ConfirmDialog
        open={deleteConfirmJobId != null}
        onCancel={() => setDeleteConfirmJobId(null)}
        onConfirm={() => {
          if (!deleteConfirmJobId) return;
          return handleDeleteJob(deleteConfirmJobId).finally(() => {
            setDeleteConfirmJobId(null);
          });
        }}
        title="Delete this conversion job?"
        description="This removes the job from your list. Download links for completed jobs may stop working."
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
