  ...
  return (
    <header className="fixed top-0 left-56 right-0 h-16 bg-midnight-card/95 backdrop-blur border-b border-midnight-border z-20 flex items-center gap-4 px-5">
      ...
      <div className="relative group">
        ...
        <div className="absolute right-0 top-10 hidden group-hover:block bg-midnight-card border border-midnight-border rounded-xl shadow-xl py-1 min-w-[140px] z-50">
          <p className="px-3 py-1.5 text-xs font-semibold text-text-primary truncate">{user?.name}</p>
          <p className="px-3 pb-1 text-[10px] text-text-muted truncate">{user?.role?.replace(/_/g, ' ')}</p>
