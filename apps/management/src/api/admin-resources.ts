/**
 * Staff admin resources (branches, devices, etc.) — `/v1/admin/*`. Not portal-scoped asset APIs.
 */
...
export type ExchangeRateStaffRef = {
  id: string;
  name: string;
  email: string;
};

export type ExchangeRateBranchRef = {
  id: string;
  name: string;
  city?: string | null;
  country?: string | null;
...
type RawExchangeRate = {
  ...
  branch?: ExchangeRateBranchRef | null;
  createdBy?: ExchangeRateStaffRef | null;
  updatedBy?: ExchangeRateStaffRef | null;
};
...
export interface ExchangeRatesSummary {
  totalRows: number;
  minUsdToBdt: number | null;
  maxUsdToBdt: number | null;
  avgUsdToBdt: number | null;
...
export function useDeleteBranch() {
  ...
  return useMutation({
    ...
    onSuccess: (_, id) => {
      qc.invalidateQueries({ queryKey: ["admin-branches"] });
      qc.invalidateQueries({ queryKey: ["admin-branch", id] });
      qc.invalidateQueries({ queryKey: ["admin-branch-members", id] });
      qc.invalidateQueries({ queryKey: ["staff-list"] });
      qc.invalidateQueries({ queryKey: ["staff-leaderboard"] });
    },
  });
...
export function useExchangeRatesSummary() {
  return useQuery<ExchangeRatesSummary>({
    queryKey: ["admin-exchange-rates-summary"],
    queryFn: async () => {
      const { data: d } = await api.get<Record<string, unknown>>(
        "/admin/exchange-rates/summary",
      );
      const mapRow = (raw: unknown): ExchangeRate | null =>
        raw && typeof raw === "object" && "id" in raw
          ? mapExchangeRate(raw as RawExchangeRate)
          : null;
      return {
        totalRows: Number(d?.totalRows ?? 0),
        minUsdToBdt:
          d?.minUsdToBdt != null ? Number(d.minUsdToBdt) : null,
        maxUsdToBdt:
          d?.maxUsdToBdt != null ? Number(d.maxUsdToBdt) : null,
        avgUsdToBdt:
          d?.avgUsdToBdt != null ? Number(d.avgUsdToBdt) : null,
        latest: mapRow(d?.latest),
        previous: mapRow(d?.previous),
        changePercentVsPrevious:
          d?.changePercentVsPrevious != null
            ? Number(d.changePercentVsPrevious)
            : null,
        totalBranches: Number(d?.totalBranches ?? 0),
      };
    },
  });
}
...
export function useCreateExchangeRate() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Partial<ExchangeRate> & { branchId?: string | null }) =>
      api.post("/admin/exchange-rates", body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-exchange-rates"] });
      qc.invalidateQueries({ queryKey: ["admin-exchange-rates-summary"] });
    },
  });
}
...
export function useUpdateExchangeRate() {
  ...
  return useMutation({
    mutationFn: ({
      id,
      ...body
    }: Partial<ExchangeRate> & { id: string; branchId?: string | null }) =>
