/**
 * Vigi integration (staff). Unrelated to client portal hosting/website routes.
 */
...
export type TplinkDiscoveredDeviceRow = {
  deviceId: string;
  alias: string;
  deviceType: string;
  deviceModel: string;
...
/** TP-Link / VIGI app credentials — lists devices on account (helps fill NVR host). */
export function useVigiCloudDiscover() {
  return useMutation({
    mutationFn: async (body: { email: string; password: string }) => {
      const { data } = await api.post<{
        devices: TplinkDiscoveredDeviceRow[];
        cloudUrlUsed: string;
        hint?: string;
      }>('/assets/vigi/cloud/discover', body);
      return data;
    },
  });
}
