...
export interface ToolsStatus {
  ...
  dataForSeo: boolean;
  /** ISO timestamp from API when status was computed */
  checkedAt?: string;
}
...
export interface SerpRankCheckResponse {
...
export function useSerpRankCheck() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (payload: {
      keyword: string;
      ...
      const { data } = await api.post<SerpRankCheckResponse>('/tools/serp/rank-check', payload, {
        timeout: 120_000,
      });
      return data;
    },
