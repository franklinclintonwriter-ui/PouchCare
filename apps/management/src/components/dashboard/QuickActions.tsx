...
function AttendanceWidget() {
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl p-3 text-white cursor-pointer transition-all hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]",
        hasCheckedOut
...
export function QuickActions() {
  ...
  return (
    <Card className="overflow-hidden" padding="none">
      <CardHeader className="min-w-0 flex-wrap items-center gap-2 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100/50 pb-3 pl-4 pr-4 pt-4 sm:flex-nowrap sm:gap-3 sm:pl-5 sm:pr-5 sm:pt-5 dark:border-gray-800 dark:from-gray-800/50 dark:to-gray-800/30">
        <CardTitle className="flex min-w-0 flex-1 items-center gap-2 text-base font-semibold leading-snug sm:text-[17px]">
          <Zap className="h-[18px] w-[18px] shrink-0 text-amber-500 sm:h-5 sm:w-5" aria-hidden />
        ...
        <Button
          ...
          className="shrink-0 text-sm text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100"
        >
          <Calendar className="mr-1.5 h-4 w-4" aria-hidden />
          <span className="hidden xl:inline">Attendance</span>
        </Button>
      </CardHeader>
      <CardContent className="!mt-0 space-y-2.5 px-4 pb-4 pt-2 sm:px-5 sm:pb-5">
        {/* Attendance Widget */}
        <AttendanceWidget />
          ...
          <div className="grid grid-cols-2 gap-2">
            {availableActions.map((action) => (
              <button
                ...
                className={cn(
                  "group flex flex-col items-center gap-1.5 rounded-xl p-2.5 text-center transition-all",
              ...
              >
                <div
                  className={cn(
                    "rounded-xl p-2 text-white transition-all group-hover:scale-110 group-hover:shadow-lg",
                    action.color,
                  )}
                >
                  {action.icon}
