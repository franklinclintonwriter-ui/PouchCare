...
import {
  useCameraStreamUrls,
  useCameraExportWindow,
  useCameraVigiSnapshotPreview,
} from '@/api/monitor';
...
export function LiveViewerModal({
  ...
  const vigiSnapshotEnabled =
    !!camera &&
    camera.status !== 'offline' &&
    streamUrls?.mode === 'vigi' &&
    !showHls;
  const vigiSnapshot = useCameraVigiSnapshotPreview(vigiSnapshotEnabled, camera?.id);
  ...
  /** Chrome/Firefox need hls.js for .m3u8; Safari can play HLS natively. */
  useEffect(() => {
    const video = hlsVideoRef.current;
    if (!showHls || !hlsPlaybackUrl || !video) return;
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_, data) => {
        if (data.fatal) {
          toast.error(
            'Could not play HLS stream. Check the URL, CORS headers on the stream server, and your network.',
          );
        }
      });
      ...
      toast.error('This browser cannot play HLS streams.');
    }
  ...
  }, [showHls, hlsPlaybackUrl, camera?.id]);
            ...
            <div className="relative z-0 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
              ...
              <div className="relative flex-1 bg-black">
                {camera!.status === 'offline' ? (
                  ...
                  <>
                    {streamUrls?.mode === 'vigi' && vigiSnapshot.blobUrl ? (
                      <div className="relative h-full w-full">
                        <img
                          src={vigiSnapshot.blobUrl}
                          alt=""
                          className="h-full w-full object-contain bg-black"
                        />
                        <div className="pointer-events-none absolute left-3 top-16 rounded-md bg-emerald-900/90 px-2 py-1 text-[10px] font-medium text-emerald-100 shadow-lg">
                          JPEG preview (NVR snapshot) · ~2s refresh · not full RTSP
                        </div>
                      </div>
                    ) : (
                      <LiveFeedPlaceholder camera={camera!} isMuted={isMuted} />
                    )}
                    {(camera.source === 'vigi' || streamUrls?.mode === 'vigi') && !vigiSnapshot.blobUrl && (
                      <div className="pointer-events-none absolute inset-0 z-10 flex items-start justify-center px-4 pt-[min(18vh,8rem)]">
                        <div className="pointer-events-auto max-w-md rounded-xl border border-white/15 bg-black/80 px-4 py-3 text-center shadow-xl backdrop-blur-md">
                          <p className="text-sm font-semibold text-white">
                            {vigiSnapshot.snapshotGaveUp
                              ? 'Snapshot preview unavailable'
                              : 'No live video in the browser (expected for VIGI)'}
                          </p>
                          {vigiSnapshot.snapshotGaveUp ? (
                            <div className="mt-2 text-left">
                              <p className="text-[11px] leading-relaxed text-red-300">
                                The API server cannot fetch JPEG snapshots from the NVR. This usually means ports{' '}
                                <strong className="text-red-200">443</strong> or{' '}
                                <strong className="text-red-200">8443</strong> (NVR web UI) are not reachable from the
                                API server — even though OpenAPI on <strong className="text-red-200">20443</strong> works
                                fine for sync.
                              </p>
                              <p className="mt-1.5 text-[11px] leading-relaxed text-gray-300">
                                <strong className="text-gray-200">Fix:</strong> ensure ports 443 and 8443 on the NVR are also
                                forwarded / reachable from the API. Or use{' '}
                                <strong className="text-gray-200">Open NVR</strong> /{' '}
                                <strong className="text-gray-200">Copy RTSP</strong> for VLC instead.
                              </p>
                              {vigiSnapshot.snapshotErr && (
                                <p className="mt-2 rounded bg-red-950/60 px-2 py-1 text-[10px] text-red-200/90">
                                  {vigiSnapshot.snapshotErr}
                                </p>
                              )}
                            </div>
                            ...
                            <>
                              <p className="mt-2 text-left text-[11px] leading-relaxed text-gray-300">
                                This integration uses <strong className="text-gray-200">RTSP</strong> on the NVR. Web
                                browsers cannot decode RTSP in a video tag. When the API can reach the NVR HTTPS port
                                (443/8443), a <strong className="text-gray-200">JPEG snapshot preview</strong> appears
                                above. Otherwise use <strong className="text-gray-200">Open NVR</strong> or{' '}
                                <strong className="text-gray-200">Copy RTSP</strong> in VLC. For HLS in this player, set an{' '}
                                <strong className="text-gray-200">.m3u8</strong> URL on the camera record.
                              </p>
                              {vigiSnapshot.snapshotErr && (
                                <p className="mt-2 text-left text-[10px] text-amber-200/95">
                                  Loading preview… {vigiSnapshot.snapshotErr}
                                </p>
                              )}
                            </>
                          )}
