...
function Header() {
  ...
  return (
    <header
      className={cn(
        'no-print',
        // Shorter bar on mobile (h-14); desktop matches sidebar (h-16)
    ...
    >
      <div className="flex h-14 items-center gap-1 px-3 lg:h-16 lg:px-5">
        {/* Mobile menu */}
        <button
          onClick={openMobile}
        ...
        {/* Inline header actions (page title / breadcrumbs intentionally not shown) */}
  ...
  switch (action.type) {
        ...
        <div className="flex rounded-md border border-gray-200 bg-gray-50 p-0.5 dark:border-gray-600/80 dark:bg-gray-800">
          {action.options.map((opt) => {
            ...
            return (
              ...
              >
                {OptIcon ? <OptIcon className="h-3 w-3 shrink-0" aria-hidden /> : null}
                <span className={cn(OptIcon ? 'hidden sm:inline' : 'inline')}>{opt.label}</span>
              </button>
            );
          })}
        </div>
    ...
    case 'button': {
      ...
      return (
        ...
        >
          <span className={showLabelOnMobile ? 'inline' : 'hidden sm:inline'}>{action.label}</span>
        </Button>
      );
    }
