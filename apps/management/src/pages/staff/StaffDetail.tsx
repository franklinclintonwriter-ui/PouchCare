      ...
      <div className="space-y-6 sm:space-y-8">
        ...
        {tab === 'overview' && (
          <div className="space-y-6 sm:space-y-8">
            ...
            {/* Skills & Expertise — full profile only */}
            {!isLimitedProfile && (member.primarySkill || member.secondarySkills || member.toolsKnown || member.certifications) && (
              <Card>
                <CardContent className="p-5 sm:p-6">
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    Skills &amp; expertise
                  ...
                  <div className="mt-4 grid gap-4 sm:grid-cols-2">
                    {member.primarySkill && (
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Primary skill</p>
                        <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">{member.primarySkill}</p>
                        {member.skillLevel && <Badge variant="primary" size="sm" className="mt-1">{member.skillLevel}</Badge>}
                      </div>
                    )}
                    {member.secondarySkills && (
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Secondary skills</p>
                        <div className="mt-1.5 flex flex-wrap gap-1.5">
                          {member.secondarySkills.split(',').map((s) => s.trim()).filter(Boolean).map((skill) => (
                            <span key={skill} className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700 dark:bg-gray-800 dark:text-gray-300">{skill}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {member.toolsKnown && (
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Tools &amp; technologies</p>
                        <div className="mt-1.5 flex flex-wrap gap-1.5">
                          {member.toolsKnown.split(',').map((t) => t.trim()).filter(Boolean).map((tool) => (
                            <span key={tool} className="rounded-full bg-primary-50 px-2 py-0.5 text-xs font-medium text-primary-700 dark:bg-primary-900/30 dark:text-primary-300">{tool}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    {member.yearsExperience != null && (
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Experience</p>
                        <p className="mt-1 text-sm font-medium text-gray-900 dark:text-gray-100">{member.yearsExperience} years</p>
                      </div>
                    )}
                    {member.certifications && (
                      <div className="sm:col-span-2">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Certifications</p>
                        <p className="mt-1 text-sm text-gray-700 dark:text-gray-300">{member.certifications}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
            ...
            {/* Personal info — full profile only */}
            {!isLimitedProfile && (member.email2 || member.whatsapp || member.address || member.country || member.emergencyContact || member.employmentType) && (
              <Card>
                <CardContent className="p-5 sm:p-6">
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    Personal &amp; HR details
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {member.email2 && (
                      <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-700/50">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Alt. email</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.email2}</p>
                      </div>
                    )}
                    {member.whatsapp && (
                      <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-700/50">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">WhatsApp</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.whatsapp}</p>
                      </div>
                    )}
                    {member.employmentType && (
                      <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-700/50">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Employment type</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.employmentType}</p>
                      </div>
                    )}
                    {member.country && (
                      <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-700/50">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Country</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.country}</p>
                      </div>
                    )}
                    {member.address && (
                      <div className="rounded-lg border border-gray-100 p-3 sm:col-span-2 dark:border-gray-700/50">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Address</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.address}</p>
                      </div>
                    )}
                    {member.emergencyContact && (
                      <div className="rounded-lg border border-rose-100 bg-rose-50/30 p-3 dark:border-rose-900/30 dark:bg-rose-950/10">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-rose-600 dark:text-rose-400">Emergency contact</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.emergencyContact}</p>
                      </div>
                    )}
                    {member.nidPassport && (
                      <div className="rounded-lg border border-gray-100 p-3 dark:border-gray-700/50">
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">NID / Passport</p>
                        <p className="mt-0.5 text-sm font-mono text-gray-900 dark:text-gray-100">{member.nidPassport}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
            ...
            {/* Online presence — full profile only */}
            {!isLimitedProfile && (member.portfolioUrl || member.linkedinUrl || member.githubUrl) && (
              <Card>
                <CardContent className="p-5 sm:p-6">
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                    Online presence
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    {member.portfolioUrl && (
                      <a href={member.portfolioUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:border-primary-300 hover:text-primary-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:border-primary-700 dark:hover:text-primary-400">
                        Portfolio
                      </a>
                    )}
                    {member.linkedinUrl && (
                      <a href={member.linkedinUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:border-blue-300 hover:text-blue-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:border-blue-700 dark:hover:text-blue-400">
                        LinkedIn
                      </a>
                    )}
                    {member.githubUrl && (
                      <a href={member.githubUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:border-gray-400 hover:text-gray-900 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:border-gray-500 dark:hover:text-gray-100">
                        GitHub
                      </a>
                    )}
                  </div>
            ...
            {!isLimitedProfile && member.terminationDate && (
              <Card className="border-l-4 border-l-rose-500">
                <CardContent className="p-5 sm:p-6">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Date</p>
                      <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">
                        {new Date(member.terminationDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>
                    </div>
                    {member.exitReason && (
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Reason</p>
                        <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{member.exitReason}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
