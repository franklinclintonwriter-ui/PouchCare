  ...
  const columns: Column<BranchMemberRow>[] = useMemo(
    () => [
      ...
      },
      {
        key: 'joinDate',
        label: 'Joined',
        render: (r) => <span className="text-xs tabular-nums text-gray-600 dark:text-gray-400">{formatDateLabel(r.joinDate)}</span>,
      },
      {
        key: 'phone',
        label: 'Contact',
      ...
      { key: 'tasksCompleted', label: 'Tasks', render: (r) => <span className="tabular-nums">{r.tasksCompleted}</span> },
      {
        key: 'rating',
        label: 'Avg rating',
        render: (r) => <span className="tabular-nums">{r.averageTaskRating != null ? r.averageTaskRating.toFixed(1) : '—'}</span>,
      },
      {
        key: 'actions',
        label: '',
      ...
      <PageTransition>
        <Card className="mx-auto max-w-lg">
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">Branch not found or you don&apos;t have access.</p>
            <Button variant="outline" className="mt-6" onClick={() => navigate('/staff/branches')}>
              Back to branches
            </Button>
          </CardContent>
        </Card>
              ...
              <div className="flex min-w-0 gap-4">
                ...
                <div className="min-w-0 space-y-2">
                  ...
                  {locationLine ? (
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Location not set — add city and country in Edit.</p>
                  )}
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                    <span className="inline-flex items-center gap-1">
                      ...
                      Created
                      {' '}
                      {formatDateLabel(branch.createdAt)}
                    </span>
                    {branch.establishedDate ? (
                      <span className="inline-flex items-center gap-1">
                        ...
                        Established
                        {' '}
                        {formatDateLabel(branch.establishedDate)}
                      </span>
                    ) : null}
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <div className="flex gap-3 rounded-xl border border-gray-100 bg-gray-50/50 p-3 dark:border-gray-700/60 dark:bg-gray-900/30">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Address</p>
                    <p className="mt-0.5 text-sm text-gray-900 dark:text-gray-100">{branch.address || '—'}</p>
                  </div>
                </div>
              </div>
              {branch.notes ? (
                <div className="sm:col-span-2 rounded-xl border border-dashed border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900/20">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Internal notes</p>
                  <p className="mt-1 whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-200">{branch.notes}</p>
                </div>
              ) : null}
            </CardContent>
          </Card>
        ...
        {managerMember ? (
          <Card className="border-primary-200/60 bg-gradient-to-br from-white to-primary-50/20 dark:border-primary-900/40 dark:from-gray-900 dark:to-primary-950/20">
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="font-semibold text-gray-900 dark:text-gray-100">{managerMember.name}</p>
          ...
          <Card>
            <CardContent className="flex items-start gap-2 py-4 text-sm text-amber-800 dark:text-amber-200/90">
              ...
              <p>
                ...
                Use Edit to adjust the name or pick someone from the team table below.
              </p>
            </CardContent>
          </Card>
        ) : null}
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {roleEntries.map(([role, n]) => {
                const pct = stats.memberCount > 0 ? (n / stats.memberCount) * 100 : 0;
                return (
                  <div key={role}>
                    <div className="mb-1 flex justify-between text-xs">
                      <span className="font-medium text-gray-700 dark:text-gray-300">{formatRole(role)}</span>
                    ...
                    <ProgressBar value={pct} max={100} size="sm" showLabel={false} color="primary" />
                  </div>
                );
              })}
            </CardContent>
...
function ContactItem({
  ...
  const content = href && value ? (
    <a href={href} className="text-sm font-medium text-primary-600 hover:underline dark:text-primary-400">
      {value}
    </a>
  ) : (
...
function ReferencesPanel({ refs }: { refs: BranchReferenceBreakdown }) {
  ...
  return (
    <Card className="h-fit border-amber-200/80 bg-amber-50/40 dark:border-amber-900/30 dark:bg-amber-950/15">
      <CardHeader>
        <CardTitle className="text-base text-amber-950 dark:text-amber-100">Data references</CardTitle>
        <p className="text-xs text-amber-900/80 dark:text-amber-200/80">
          {refs.total}
          {' '}
          record(s) reference this branch name across linked data.
        </p>
      </CardHeader>
      <CardContent>
          ...
          {lines.map((r) => {
            ...
            return (
              ...
              >
                ...
                <span className="shrink-0 tabular-nums text-sm font-semibold text-amber-950 dark:text-amber-50">{r.n}</span>
              </li>
            );
          })}
        </ul>
