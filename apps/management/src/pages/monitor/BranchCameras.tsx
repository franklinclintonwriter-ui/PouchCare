...
import { useVigiIntegration } from "@/api/vigi";
...
const CameraGridTile = forwardRef<
  HTMLDivElement,
  {
    camera: CameraDevice;
    onClick: () => void;
    onPing?: (e: ReactMouseEvent) => void;
    pingPending?: boolean;
  }
>(function CameraGridTile({ camera, onClick, onPing, pingPending }, ref) {
  return (
    <motion.div
      ref={ref}
      layout
          ...
          <div className="flex items-end justify-between gap-1">
            <div className="min-w-0">
              ...
              {camera.lastMotionAt && camera.hasMotionDetect && (
                <p className="truncate text-[9px] text-blue-300/90">
                  Motion {formatMotionAge(camera.lastMotionAt)}
                </p>
              )}
            </div>
              ...
              {onPing && (
                <span
                  ...
                  onKeyDown={(e: ReactKeyboardEvent) => {
                    ...
                    e.preventDefault();
                    e.stopPropagation();
                    onPing(e as unknown as ReactMouseEvent);
                  }}
                  aria-disabled={pingPending}
                  className={cn(
                    "rounded bg-black/50 px-1.5 py-0.5 text-[9px] font-semibold text-blue-300 backdrop-blur-sm hover:bg-black/70",
                    pingPending && "pointer-events-none opacity-50",
                  )}
...
function CameraListRow({
  ...
  const lastMotionStr = camera.lastMotionAt
    ? new Date(camera.lastMotionAt).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "—";
      ...
      <div className="flex shrink-0 flex-row items-center justify-between gap-2 sm:flex-col sm:items-end sm:justify-start sm:gap-1.5">
        <div className="flex flex-wrap items-center justify-end gap-1.5">
          ...
          {onPing && (
            ...
            >
              <Signal
                className={cn("h-3.5 w-3.5", pingPending && "animate-pulse")}
              />
            </button>
          )}
        </div>
...
function BranchStatsBar({
  ...
  const pct =
    cameras.length > 0 ? Math.round((online / cameras.length) * 100) : 0;
...
// ── Vigi NVR inline status banner ─────────────────────────

function VigiNvrStatusCard({
  branchId,
  onOpenSettings,
}: {
  branchId: string;
  ...
  const btnCls =
    "flex shrink-0 items-center gap-1.5 self-start rounded-lg border px-3 py-1.5 text-xs font-semibold transition-colors sm:self-auto";

  if (!integration) {
    return (
      <div className="flex flex-col gap-3 rounded-xl border border-amber-200/80 bg-amber-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-amber-800/40 dark:bg-amber-950/20">
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400">
              Connect a TP-Link VIGI NVR to automatically import and sync camera
              channels.
            </p>
          </div>
        </div>
        <button
          ...
          className={cn(
            btnCls,
            "border-amber-300 bg-white text-amber-900 hover:bg-amber-50 dark:border-amber-700 dark:bg-amber-950/40 dark:text-amber-200 dark:hover:bg-amber-900/40",
          )}
        >
  ...
  if (!integration.hasPassword || !integration.enabled) {
    return (
      <div className="flex flex-col gap-3 rounded-xl border border-amber-200/80 bg-amber-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-amber-800/40 dark:bg-amber-950/20">
        <div className="flex items-start gap-2.5">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-900 dark:text-amber-200">
              {!integration.hasPassword
                ? "NVR password required"
                : "NVR integration disabled"}
            </p>
            <p className="text-xs text-amber-700 dark:text-amber-400">
              NVR at{" "}
              <span className="rounded bg-amber-100 px-1 font-mono text-[11px] dark:bg-amber-900/40">
                {integration.host}:{integration.port}
              </span>{" "}
              (user: <span className="font-mono">{integration.username}</span>){" "}
              —{" "}
              {!integration.hasPassword
                ? "enter the NVR password in Settings to enable sync."
                : "enable the integration in Settings."}
            </p>
          </div>
        </div>
        <button
        ...
        >
          ...
          {!integration.hasPassword ? "Set password" : "Enable NVR"}
        </button>
      </div>
    );
  }
  ...
  if (integration.lastError) {
    const err = integration.lastError;
    const unreachable =
      /ETIMEDOUT|ECONNREFUSED|ENOTFOUND|EHOSTUNREACH/i.test(err) ||
      /network|unreachable/i.test(err);
    return (
      <div className="flex flex-col gap-3 rounded-xl border border-red-200/80 bg-red-50/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-red-900/40 dark:bg-red-950/20">
        <div className="flex min-w-0 items-start gap-2.5">
          <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-red-600 dark:text-red-400" />
          <div className="min-w-0">
            <p className="text-sm font-semibold text-red-900 dark:text-red-200">
              NVR connection error
            </p>
            <p
            ...
            {unreachable && (
              <p className="mt-1.5 text-[11px] leading-snug text-red-800/95 dark:text-red-300/95">
                The API cannot reach that address. Private IPs (192.168.x.x) only work if the API runs on the same LAN or you use VPN/tunnel/DDNS to the NVR.
              </p>
            )}
          </div>
        </div>
        <button
          ...
          className={cn(
            btnCls,
            "border-red-200 bg-white text-red-800 hover:bg-red-50 dark:border-red-800 dark:bg-red-950/40 dark:text-red-200 dark:hover:bg-red-900/40",
          )}
        >
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-200/80 bg-emerald-50/60 px-4 py-3 dark:border-emerald-900/40 dark:bg-emerald-950/20">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p className="text-xs text-emerald-700 dark:text-emerald-400">
            <span className="rounded bg-emerald-100 px-1 font-mono text-[11px] dark:bg-emerald-900/40">
              {integration.host}:{integration.port}
            </span>{" "}
            · user: <span className="font-mono">{integration.username}</span> ·
            last sync: {syncLabel}
      ...
      <button
        ...
        className={cn(
          btnCls,
          "border-emerald-200 bg-white text-emerald-800 hover:bg-emerald-50 dark:border-emerald-800/60 dark:bg-emerald-950/40 dark:text-emerald-200 dark:hover:bg-emerald-900/40",
        )}
      >
        <Server className="h-3.5 w-3.5" />
        NVR settings
...
export default function BranchCameras() {
  ...
  const [sourceFilter, setSourceFilter] = useState<"all" | "manual" | "vigi">(
    "all",
  );
  const [sortKey, setSortKey] =
    useState<NonNullable<CamerasByBranchParams["sort"]>>("label_asc");
  ...
  const handleCameraPing = useCallback(
    (cameraId: string, e: React.MouseEvent) => {
      ...
      pingMutation.mutate(cameraId, {
        ...
        onError: (err) =>
          toast.error(
            err instanceof Error ? err.message : "Could not record feed check",
          ),
  ...
  const handleRefresh = useCallback(async () => {
    ...
    await Promise.all([
      ...
      branchId
        ? queryClient.invalidateQueries({ queryKey: ["vigi-integration", branchId] })
        : Promise.resolve(),
    ]);
      ...
      () => ({
        ...
        actions: [
          ...
          },
          {
            ...
            onChange: (v: string) =>
              setSortKey(
                (v === "" ? "label_asc" : v) as NonNullable<
                  CamerasByBranchParams["sort"]
                >,
              ),
          },
          {
  ...
  const pageLoading =
    (branchLoading || camerasLoading) && !displayBranch && cameras.length === 0;
  ...
  return (
    <PageTransition className="space-y-3 sm:space-y-5 pb-[max(1rem,env(safe-area-inset-bottom))]">
      ...
      {/* Branch location strip */}
      {displayBranch && (displayBranch.city || displayBranch.country) && (
        <div className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
          <MapPin className="h-3.5 w-3.5 shrink-0" />
          {[displayBranch.city, displayBranch.country]
            .filter(Boolean)
            .join(", ")}
        </div>
      )}
      ...
      {viewMode === "multiview" && (
        ...
        >
          <div className="grid h-full grid-cols-2 grid-rows-2 gap-0.5 bg-gray-950 p-0.5">
            {cameras.slice(0, 4).map((cam, i) => (
              <button
                ...
                className={cn(
                  ...
                  cam.status !== "offline" &&
                    "hover:ring-2 hover:ring-inset hover:ring-blue-500/60",
                )}
              >
            ...
            {/* Filler cells if < 4 */}
            {Array.from({ length: Math.max(0, 4 - cameras.length) }).map(
              (_, i) => (
                <div
                  key={`empty-${i}`}
      ...
      >
        <div className="space-y-4">
          <p className="text-sm leading-relaxed text-gray-600 dark:text-gray-300">
            ...
            connect the NVR, save the integration, then use{" "}
            <strong>Sync cameras</strong> to pull channels. Manual camera
            records are maintained through your existing admin or API workflows.
          </p>
          {branchId ? (
            <VigiIntegrationCard
              branchId={branchId}
              canManage={canVigiManage}
            />
          ) : (
