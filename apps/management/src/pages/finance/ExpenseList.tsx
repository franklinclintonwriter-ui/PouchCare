...
export default function ExpenseList() {
  ...
  const stats = useMemo(() => {
    ...
    const approved = allExpenses
      .filter((e) =>
        ["APPROVED_MGR", "VERIFIED", "APPROVED", "PAID"].includes(e.status),
      )
      .reduce((s, e) => s + e.amount, 0);
    const pending = allExpenses
      .filter((e) => ["WAITING", "SUBMITTED", "PENDING"].includes(e.status))
      .reduce((s, e) => s + e.amount, 0);
  ...
  const columns: Column<Expense>[] = [
    ...
    },
    {
      ...
      label: "Category",
      render: (row) => (
        <Badge variant={categoryVariants[row.category] ?? "default"}>
          {row.category}
        </Badge>
      ),
    },
    ...
    <PageTransition className="space-y-6">
      <StatsRow
        ...
        items={[
          {
            title: "Total MTD",
            value: formatCurrency(stats.totalMTD),
                ...
                {expenses.map((exp) => (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center justify-between gap-3 border-t border-gray-100 pt-4 text-xs dark:border-gray-700/60">
                      <Badge
                        variant={categoryVariants[exp.category] ?? "default"}
                        size="sm"
                      >
                        {exp.category}
                      </Badge>
              ...
              onClick={async () => {
                ...
                try {
                  await createExpense.mutateAsync({
                    ...
                    expenseDate: expDate
                      ? new Date(`${expDate}T12:00:00Z`).toISOString()
