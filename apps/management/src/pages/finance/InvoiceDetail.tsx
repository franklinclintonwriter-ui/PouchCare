...
function normalizeInvoiceStatus(value: string) {
  ...
  if (
    ["UNPAID", "PARTIAL", "PAID", "OVERDUE", "REFUNDED", "CANCELLED"].includes(
      v,
    )
  )
    return v;
    ...
    try {
      await updateInvoice.mutateAsync({
        ...
        dueDate: form.dueDate
          ? new Date(`${form.dueDate}T12:00:00Z`).toISOString()
          : undefined,
      });
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <InfoRow
              label="Issue Date"
              value={new Date(invoice.issueDate).toLocaleDateString("en-GB", {
                day: "numeric",
                month: "short",
                year: "numeric",
              })}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button isLoading={updateInvoice.isPending} onClick={handleSave}>
              Save Changes
            </Button>
          </>
        }
...
function InfoRow({
  ...
  return (
    <div
      className={`flex items-center justify-between${border ? " border-b border-gray-100 pb-2 dark:border-gray-700" : ""}`}
    >
