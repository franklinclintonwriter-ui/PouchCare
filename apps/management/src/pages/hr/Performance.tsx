...
export default function Performance() {
  ...
  const performanceStats = useMemo(() => {
    ...
    const avgScore =
      totalReviews > 0
        ? Math.round(
            reviewData.reduce((s, r) => s + r.overallScore, 0) / totalReviews,
          )
        : 0;
    const topScorer =
      reviewData.length > 0
        ? ([...reviewData].sort((a, b) => b.overallScore - a.overallScore)[0]
            ?.staffName ?? "—")
        : "—";
    const positiveCount = reviewData.filter((r) => r.trend >= 0).length;
    return [
      {
        title: "Total Reviews",
        value: totalReviews,
  ...
  return (
    <PageTransition className="space-y-6">
      <StatsRow items={performanceStats} loading={isLoading} />
...
function PerformanceCard({
  ...
  return (
    <Card hover>
      ...
      {review.comments && (
        <p className="mt-2.5 line-clamp-2 text-xs text-gray-500 dark:text-gray-400 italic">
