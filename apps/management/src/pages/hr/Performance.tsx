...
import { usePerformanceReviews, useCreatePerformanceReview, useUpdatePerformanceReview, useDeletePerformanceReview } from '@/api/performance';
...
const QUARTER_OPTIONS = ['Q1', 'Q2', 'Q3', 'Q4'].map((q) => ({ label: q, value: q }));
...
export default function Performance() {
  ...
  const openEdit = useCallback((review: PerformanceReview) => {
    ...
    setForm({
      ...
      reviewYear: review.period.split(' ')[1] ?? String(new Date().getFullYear()),
  ...
  useHeaderConfig(useMemo(() => ({
    ...
    actions: canCreate
      ? [{ type: 'button' as const, label: 'Add Review', icon: Plus, onClick: () => { setEditingId(null); setForm(emptyForm); setModalOpen(true); } }]
      : [],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : `Failed to ${editingId ? 'update' : 'create'} review`);
    }
  };
      ...
      <Modal
        ...
        footer={(
          <>
            ...
            <Button size="sm" isLoading={createReview.isPending || updateReview.isPending} onClick={handleSave}>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              ...
              onChange={(e) => setForm(f => ({ ...f, reviewYear: e.target.value }))}
            />
          </div>
          <Input
            type="number"
...
function PerformanceCard({ review, canManage, onEdit, onDelete }: {
    ...
    <Card hover>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-gray-900 dark:text-gray-100">{review.staffName}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1.5">
        {scoreEntries.map(([key, val]) => (
          <div key={key} className="flex items-center gap-2">
            <span className="w-14 truncate text-[10px] font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">{key}</span>
            ...
            <span className="w-6 text-right text-[10px] font-semibold text-gray-600 dark:text-gray-400">{val}</span>
          </div>
        ))}
      </div>
