...
export default function FaviconToolPage() {
  ...
  const clearAll = useCallback(() => setFile(null), []);
  ...
  const headerConfig = useMemo(
    ...
    [file, clearAll, run, running],
  );

  useHeaderConfig(headerConfig, [clearAll, run, file, running]);

  return (
    <PageTransition>
      <div className="mx-auto max-w-5xl space-y-5 px-4 pb-10 pt-1 sm:px-6 lg:px-8">
        <ToolDisclaimer status={status ?? undefined} compact />

        <ToolPageChrome
          accent="slate"
          eyebrow="Assets · Sharp"
          title="Favicon package"
          hint="Square source → multi-size PNG ZIP for production."
        />

        <ToolRunPanel onRun={run} showActions={false} runLabel="Build & download ZIP" running={running}>
          <div>
            ...
            <p className="mt-1.5 flex items-center gap-1.5 text-[11px] text-[var(--color-text-secondary)]">
              ...
              16–180px PNG · server-side Sharp · ~4MB max
            </p>
          </div>
        </ToolRunPanel>
