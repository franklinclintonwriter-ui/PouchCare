...
export default function SerpTop100ToolPage() {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const out = await serpMutation.mutateAsync({
        keyword: k,
        gl,
        hl,
        num: 100,
  ...
  }, [keyword, market, location, googleDomain, push, serpMutation]);
  ...
  const headerConfig = useMemo(
    () => ({
      title: 'SERP Top 100',
      description: filtered.length ? `${filtered.length} rows` : undefined,
    ...
    [filtered.length, exportCsv, clearAll, run, keyword, status?.serpApi, running],
  );

  useHeaderConfig(headerConfig, [exportCsv, clearAll, run, filtered.length, running, keyword, status?.serpApi]);
    ...
    <PageTransition>
      <div className="mx-auto max-w-6xl space-y-5 px-4 pb-10 pt-1 sm:px-6 lg:px-8">
        ...
        <ToolPageChrome
          accent="sky"
          eyebrow="SEO · SerpAPI"
          title="Organic SERP — top 100"
          hint="Market, optional geo — filters apply to the grid below."
        />
        ...
        {(running || (doneOnce && allRows.length > 0)) && (
          <div
            ...
            role="status"
          >
            {running ? 'Fetching organic results…' : `${allRows.length} results`}
          </div>
        )}

        <ToolRunPanel onRun={run} showActions={false} runLabel="Fetch SERP" running={running}>
          ...
          <div className="border-t border-gray-100 pt-3 dark:border-gray-700/60">
            <button
              type="button"
              onClick={() => setSerpAdvancedOpen((v) => !v)}
              className="flex w-full items-center justify-between gap-2 text-left text-xs font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400"
            >
              <span>Localization (optional)</span>
              {serpAdvancedOpen ? <ChevronUp className="h-4 w-4 shrink-0" /> : <ChevronDown className="h-4 w-4 shrink-0" />}
            </button>
            {serpAdvancedOpen && (
        ...
        {allRows.length > 0 && (
          <>
            <div>
              <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Signal</p>
            ...
            <DataTable
              ...
              emptyDescription="Enter a keyword to fetch results."
            />
          </>
        )}
      </div>
