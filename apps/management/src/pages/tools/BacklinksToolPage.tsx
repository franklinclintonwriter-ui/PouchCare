...
import { Link2, Download, Play, Trash2, ChevronRight } from 'lucide-react';
...
export default function BacklinksToolPage() {
  ...
  const headerConfig = useMemo(
    () => ({
      title: 'Backlinks',
      description: rows.length ? `${rows.length} domains` : undefined,
    ...
    [rows.length, exportCsv, clearAll, run, url, status?.dataForSeo, running],
  );

  useHeaderConfig(headerConfig, [exportCsv, clearAll, run, rows.length, running, url, status?.dataForSeo]);
    ...
    <PageTransition>
      <div className="mx-auto max-w-6xl space-y-5 px-4 pb-10 pt-1 sm:px-6 lg:px-8">
        ...
        <ToolPageChrome
          ...
          hint="Referring domains with authority signals — export from the toolbar above."
        />
        ...
        <ToolRunPanel onRun={run} showActions={false} runLabel="Analyze" running={running}>
          ...
          {!status?.dataForSeo && (
            <p className="text-xs text-amber-700 dark:text-amber-300">
              Set <code className="rounded bg-amber-100 px-1 font-mono dark:bg-amber-900/50">DATAFORSEO_*</code> on the API.
            </p>
          )}
        </ToolRunPanel>
        ...
        {rows.length > 0 && (
          <>
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200/80 bg-white dark:border-gray-700/60 dark:bg-gray-800/80">
              <DataTable columns={columns} data={rows} compact emptyTitle="No backlinks found" emptyDescription="Try a different domain." />
              {expandedId ? (
                <ExpandableRowDetail
                  expanded
                  onToggle={() => setExpandedId(null)}
                  label="Subdomain breakdown"
                >
                  {(() => {
