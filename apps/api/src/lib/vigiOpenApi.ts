...
/** RFC 6066: SNI must not use raw IP literals; wrong SNI can break TLS to some NVR firmware. */
export function tlsServerNameForHost(host: string): string | undefined {
  return net.isIP(host) !== 0 ? undefined : host
}
...
function httpsJson(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.setTimeout(t, () => {
      ...
      reject(
        new Error(
          `VIGI HTTPS timeout after ${t}ms (${host}:${port}${pathWithQuery}). ` +
            `No TCP/TLS response from NVR — the API host cannot reach this address (routing/VPN/firewall), ` +
            `or OpenAPI is disabled. From the API PC run: Test-NetConnection ${host} -Port ${port}. ` +
            `Optional: set VIGI_HTTPS_TIMEOUT_MS in apps/api/.env for very slow links.`,
        ),
      )
    })
    req.on('error', reject)
