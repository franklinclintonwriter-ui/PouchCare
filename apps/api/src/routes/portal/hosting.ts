...
/**
 * CLIENT PORTAL — domain hosting & DNS for **end customers** (authenticated portal members).
 * Scoped with `portalMemberId`. This is the customer-facing product surface on pouchcare.com.
 *
 * Do not confuse with staff routes in `routes/assets/index.ts` (`/v1/assets/domains`), which are
 * for PouchCare **internal** operations inventory in the Management dashboard.
 */
const router = Router();
...
router.get("/search", async (req, res) => {
  try {
    ...
    // If Name.com credentials are configured, try the real API; fall back on error
    if (env.NAMECOM_USERNAME && env.NAMECOM_TOKEN) {
      try {
        ...
        // Log the credential error but degrade gracefully to mock results
        console.warn("[portal/hosting/search] Name.com API unavailable, using mock results:", (namcomErr as Error).message);
      }
    }

    // Fallback: mock results when no Name.com credentials or API is unreachable
