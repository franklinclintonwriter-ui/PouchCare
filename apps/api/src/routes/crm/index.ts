...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (stage || status)
        filter.stage = (stage ||
          status) as Prisma.EnumLeadStageFilter["equals"];
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      const allowed = await canAccessCrmLead(
        req.user!.id,
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const {
        ...
        nextFollowUpDate,
        lastContactDate,
      } = req.body as z.infer<typeof updateLeadSchema>;
    ...
    try {
      ...
      if (req.user!.role === "BRANCH_MANAGER") {
        ...
        if (!branch)
          return forbidden(
            res,
            "Branch is required for your account to create orders",
          );
      }
...
router.get(
  ...
  async (req: AuthRequest, res) => {
    try {
      const allowed = await canAccessSalesOrder(
        req.user!.id,
        req.user!.role as SystemRole,
        req.params.id,
      );
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      const {
        ...
        deadline,
        deliveryLink,
      } = req.body as z.infer<typeof updateOrderSchema>;
      const data: Record<string, any> = {};
