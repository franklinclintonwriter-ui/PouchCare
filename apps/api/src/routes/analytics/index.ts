...
function calcTrend(current: number, previous: number): { change: number; direction: 'up' | 'down' | 'neutral' } {
  if (previous === 0) return { change: current > 0 ? 100 : 0, direction: current > 0 ? 'up' : 'neutral' }
  const change = Math.round(((current - previous) / previous) * 100)
  return { change: Math.abs(change), direction: change > 0 ? 'up' : change < 0 ? 'down' : 'neutral' }
}
...
router.get('/summary', async (_, res) => {
  try {
    ...
    const [
      ...
      prisma.task.count({ where: { approvalStatus: 'VERIFIED', updatedAt: { gte: lastMonthStart, lte: lastMonthEnd } } }),
      // Attendance
      prisma.attendance.count({ where: { status: 'PRESENT', date: { gte: today } } }),
      prisma.attendance.count({ where: { status: 'PRESENT', date: { gte: new Date(today.getTime() - 86400000), lt: today } } }),
      // Staff
      ...
      prisma.staffMember.count({ where: { status: { equals: 'Active', mode: 'insensitive' }, createdAt: { gte: lastMonthStart, lte: lastMonthEnd } } }),
      // Clients
      prisma.portalMember.count(),
      prisma.portalMember.count({ where: { status: 'ACTIVE' } }),
      prisma.portalMember.count({ where: { registrationDate: { gte: thisMonth } } }),
      prisma.portalMember.count({ where: { registrationDate: { gte: lastMonthStart, lte: lastMonthEnd } } }),
      // Pipeline
      prisma.crmLead.count({ where: { stage: 'WON' } }),
      prisma.crmLead.count(),
      // Revenue
      prisma.monthlyRevenue.findMany({ where: { year: currentYear }, orderBy: { month: 'asc' } }),
      prisma.monthlyRevenue.findMany({ where: { year: currentYear - 1 }, orderBy: { month: 'asc' } }),
      // Recent activities
      prisma.task.findMany({
        where: { updatedAt: { gte: new Date(Date.now() - 7 * 86400000) } },
        orderBy: { updatedAt: 'desc' },
        take: 5,
        select: { id: true, title: true, status: true, updatedAt: true, assignedMember: { select: { name: true } } },
      }),
      prisma.attendance.findMany({
        where: { date: { gte: today } },
        orderBy: { checkInTime: 'desc' },
        take: 5,
        select: { id: true, staffMember: { select: { name: true } }, status: true, checkInTime: true },
      }),
      prisma.crmLead.findMany({
        where: { updatedAt: { gte: new Date(Date.now() - 7 * 86400000) } },
        orderBy: { updatedAt: 'desc' },
        take: 5,
        select: { id: true, company: true, stage: true, estimatedValue: true, updatedAt: true },
      }),
      // Leaderboards
    ...
    const attendScore = staffActive > 0 ? Math.min(100, (presentToday / staffActive) * 100) : 100
    ...
    const healthTotal = Math.round((taskScore * 0.25) + (attendScore * 0.20) + (pipeScore * 0.25) + (clientScore * 0.15) + 75 * 0.15)

    // Revenue calculations
    const totalRevenue = revenueData.reduce((s, r) => s + r.totalRevenueUsd, 0)
    const totalExpenses = revenueData.reduce((s, r) => s + (r.totalExpensesUsd ?? 0), 0)
    const lastYearTotal = lastYearRevenue.reduce((s, r) => s + r.totalRevenueUsd, 0)
    ...
    return ok(res, {
      ...
      kpis: {
        ...
        attendance: {
          ...
          percentage: staffActive > 0 ? Math.round((presentToday / staffActive) * 100) : 0,
          trend: attendTrend,
        },
        clients: {
          total: clientsTotal,
        ...
        pipeline: {
          won: pipelineWon,
          total: pipelineTotal,
          winRate: pipelineTotal > 0 ? Math.round((pipelineWon / pipelineTotal) * 100) : 0,
        },
      },
      revenue: {
        ...
        summary: {
          ...
          avgMonthly: revenueData.length > 0 ? Math.round(totalRevenue / revenueData.length) : 0,
        },
      },
      activities: {
        tasks: recentTasks.map((t) => ({
          ...
          title: t.title,
          status: t.status,
          subtitle: t.assignedMember?.name ? `Assigned · ${t.assignedMember.name}` : 'Unassigned',
          time: t.updatedAt instanceof Date ? t.updatedAt.toISOString() : String(t.updatedAt),
        })),
        attendance: recentAttendance.map((a) => ({
          ...
          status: a.status,
          time: a.checkInTime instanceof Date ? a.checkInTime.toISOString() : String(a.checkInTime),
        })),
        leads: recentLeads.map((l) => ({
          ...
          subtitle:
            l.estimatedValue != null && Number(l.estimatedValue) > 0
              ...
              : undefined,
          time: l.updatedAt instanceof Date ? l.updatedAt.toISOString() : String(l.updatedAt),
        })),
      },
      leaderboards: {
        staff: topStaff,
  ...
  try {
    ...
    for (let i = 0; i < recent.length - 1; i++) {
      if (recent[i + 1].totalRevenueUsd > 0) {
        growthRates.push((recent[i].totalRevenueUsd - recent[i + 1].totalRevenueUsd) / recent[i + 1].totalRevenueUsd)
      }
    }
...
router.get('/tasks', async (_, res) => {
  try {
    ...
    const [total, done, inProgress, pending, overdue, thisMonthDone, lastMonthDone] = await Promise.all([
      prisma.task.count(),
      prisma.task.count({ where: { approvalStatus: 'VERIFIED' } }),
      prisma.task.count({ where: { status: 'IN_PROGRESS' } }),
      prisma.task.count({ where: { status: 'NOT_STARTED' } }),
      prisma.task.count({ where: { status: { not: 'DONE' }, deadline: { lt: new Date() } } }),
      prisma.task.count({ where: { approvalStatus: 'VERIFIED', updatedAt: { gte: thisMonth } } }),
      prisma.task.count({ where: { approvalStatus: 'VERIFIED', updatedAt: { gte: lastMonthStart, lte: lastMonthEnd } } }),
    ])
