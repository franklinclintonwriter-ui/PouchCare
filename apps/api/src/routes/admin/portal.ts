...
router.post('/members/:id/avatar', avatarUpload.single('file'), async (req, res) => {
  try {
    const file = req.file
    if (!file) return badRequest(res, 'No file')
    const member = await prisma.portalMember.findUnique({ where: { id: req.params.id }, select: { id: true } })
