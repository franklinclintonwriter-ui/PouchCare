...
import { decryptCredential } from '@/lib/credentialsCrypto'
import { buildVigiLiveRtsp, buildVigiReplayRtsp } from '@/lib/vigiRtsp'
import { fetchVigiSnapshotJpeg } from '@/lib/vigiSnapshotHttp'
 ...
 * Proxies one JPEG frame from the VIGI NVR snapshot URL (HTTPS + Digest) so the browser can show a live preview
 * without RTSP. Tries HTTPS snapshot on the OpenAPI port first, then 443/8443. Same reachability as sync.
 */
router.get('/cameras/:cameraId/snapshot', requirePermission('monitor.view'), async (req: AuthRequest, res) => {
  try {
    ...
    if (!integration || cam.source !== 'vigi' || cam.vigiChannel == null) {
      return badRequest(res, 'Snapshot preview is only available for cameras imported from a VIGI NVR')
    }

    const password = decryptCredential(integration.passwordEncrypted)
    const { buffer } = await fetchVigiSnapshotJpeg({
      host: integration.host,
      username: integration.username,
      password,
      channel: cam.vigiChannel,
      tlsAllowInsecure: integration.tlsAllowInsecure,
