...
async function seedCameras() {
  ...
  // ── Vigi NVR integration (placeholder row; real credentials via Management Monitor UI or `npm run vigi:upsert-local`) ──
  // Example LAN: 192.168.31.x (see device General Settings). TP-Link Open API: HTTPS port 20443.
  const nvr = await prisma.vigiNvrIntegration.create({
    data: {
      ...
      port: 20443,
      username: "admin",
      passwordEncrypted: "enc:CHANGE_ME", // ← replace via UI or vigi:upsert-local (invalid until updated)
  ...
  for (let i = 0; i < cameras.length; i++) {
    ...
    await prisma.cameraDevice.create({
      data: {
        ...
        nvrDevice: "TP-Link VIGI NVR1016H",
        rtspUrl: `rtsp://192.168.31.228:554/stream${i + 1}`,
...
// ─────────────────────────────────────────────────────────────────────────────
// ClientSegment — saved filter presets for the Admin Panel Clients list.
// Requires migration 20260417130000_client_segments to be applied.
// Survives table-missing (the whole function no-ops with a console note).
// ─────────────────────────────────────────────────────────────────────────────
async function seedClientSegments() {
  const presets: Array<{ name: string; params: Record<string, unknown> }> = [
    { name: "Active", params: { status: "ACTIVE" } },
    { name: "Pending verification", params: { status: "PENDING" } },
    { name: "Suspended", params: { status: "SUSPENDED" } },
    { name: "Bangladesh", params: { country: "BD" } },
    { name: "UAE", params: { country: "AE" } },
    { name: "Portugal", params: { country: "PT" } },
  ];

  let inserted = 0;
  for (const p of presets) {
    try {
      await (prisma as any).clientSegment.upsert({
        where: { name: p.name },
        update: { params: p.params },
        create: { name: p.name, params: p.params },
      });
      inserted++;
    } catch (err: any) {
      if (String(err?.message || "").includes("does not exist")) {
        console.log(
          "ℹ️  ClientSegment table missing — skipping seed. Apply migration 20260417130000_client_segments.",
        );
        return;
      }
      throw err;
  ...
  console.log(`✅ Client segments seeded (${inserted} presets)`);
}
